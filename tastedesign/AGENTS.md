...
## ARC Engine — Node Connection Rules

When working with the ARC Engine node graph (`src/app/arc-engine/`), every node must be properly connected to form a valid directed acyclic graph (DAG). Nodes that are not connected will produce empty output or errors.

### Connection Requirements

| Node Category | Inputs | Connection Required |
|---------------|--------|-------------------|
| **Source** (background, image-loader, video-loader, text) | 0 inputs | ❌ No — generan contenido propio |
| **Effect** (blur, glow, color-grade, etc.) | 1 input (Background) | ✅ Sí — deben recibir entrada de otro nodo |
| **Keyer** (chroma-keyer, luma-keyer, etc.) | 1 input (Background) | ✅ Sí |
| **Distort** (displacement, lens-distort, ripple, etc.) | 1-2 inputs | ✅ Sí |
| **Mask** (mask-rectangle, mask-ellipse, etc.) | 0-2 inputs | ❌ No para generadores de forma, ✅ Sí para procesadores |
| **Composite** (merge, multimerge) | 2+ inputs | ✅ Sí — todos los inputs deben estar conectados |
| **3D** (shape-3d, renderer-3d, etc.) | 0-2 inputs | ❌ No para primitivas, ✅ Sí para renderer-3d |

### Validación Automática

El módulo `src/app/arc-engine/graph-validator.ts` proporciona:

- `validateConnections(nodes)` — verifica que todos los nodos no-fuente tengan inputs conectados
- `topologicalSort(nodes)` — ordena nodos en orden de dependencia (Kahn's algorithm)
- `prepareGraph(nodes)` — función combinada que valida + ordena en un solo paso

### Reglas para Agentes IA

1. **Antes de renderizar**, llama a `prepareGraph(nodes)` para validar conexiones
2. **Si hay errores**, corrige las conexiones — no intentes renderizar un grafo inválido
3. **Los Source nodes** (0 inputs) son los únicos que pueden estar sin conexión
4. **Ciclos** están prohibidos — el grafo DEBE ser acíclico
5. **Render order**: los source nodes se renderizan primero, luego los efectos que dependen de ellos, y finalmente los composite nodes
6. **Nodos flotantes** (sin conexión de entrada ni de salida) generan una advertencia y no se renderizan
...
```
Background ──► Blur ──► Glow ──► Merge ──► Output
  (source)      (effect)  (effect)  (composite)
```

### Flujo de Render

```typescript
import { prepareGraph } from './arc-engine/graph-validator';

// 1. Validar conexiones
const { sorted, errors, warnings } = prepareGraph(nodes);

// 2. Si hay errores, detener
if (errors.length > 0) {
  console.error('Errores de conexión:', errors);
  return;
}

// 3. Renderizar en orden topológico
...
### Verificación de Node Connections

- [ ] Todos los nodos no-fuente tienen inputs conectados
- [ ] No hay ciclos en el grafo
- [ ] El orden topológico se calcula correctamente
- [ ] Los nodos source se renderizan primero
- [ ] Los nodos flotantes generan advertencia (no error silencioso)
