...
from download import download, download_subtitles_only, is_url  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument(
        "--subtitles-only",
        action="store_true",
        help="Download subtitles only — no video download or frame extraction.",
    )
    ap.add_argument(
        "--no-whisper",
        action="store_true",
    ...
    if args.subtitles_only:
        if not is_url(args.source):
            raise SystemExit("--subtitles-only requires a URL (not a local file)")
        print("[watch] subtitles-only mode — downloading captions…", file=sys.stderr)
        dl = download_subtitles_only(args.source, work / "subtitles")
        ...
        if dl.get("subtitle_path"):
            try:
                ...
                if args.start or args.end:
                    target_segments = filter_range(all_segments, parse_time(args.start), parse_time(args.end))
                transcript_segments = target_segments
        ...
        info = dl.get("info") or {}
        full_duration = info.get("duration") or 0

        print()
        print("# watch: subtitles report")
        print()
        ...
        print("- **Mode:** subtitles only (no video download)")
        ...
        if transcript_text:
            ...
            print("## Transcript")
            print()
            print("_No transcript available — this video has no captions._")
    ...
    # ── FULL MODE (video + frames + transcript) ─────────────────────────
